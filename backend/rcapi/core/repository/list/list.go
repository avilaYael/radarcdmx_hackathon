...
func (i *Implementation) BuildListEntityQuery(
	...
	if request.GetJoinParams() != nil {
		...
		if onlyCount {
			pks := entity.PrimaryKeyIdentifiers()
			if len(pks) == 1 {
				return fmt.Sprintf(
			...
			return fmt.Sprintf(
				"SELECT DISTINCT count(*) FROM %s %s %s WHERE %s",
				entity.EntityIdentifier(),
				request.GetJoinParams().JoinStatement,
				jsonTablesFinal,
				finalWhereClause,
	...
	if onlyCount {
		...
		return fmt.Sprintf(
			"SELECT DISTINCT count(*) FROM %s %s WHERE %s",
			entity.EntityIdentifier(),
			jsonTablesFinal,
			whereClause,
		), nil
